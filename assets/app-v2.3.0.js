...
(() => {
  ...
  function filterRoutes() {
    ...
    let result = routes.filter((route) => {
      ...
      if (quickFilter === 'loop') return isLoopLike(route);
  ...
  function openRoute(routeId) {
    ...
    <div class="detail-grid">
      ${detailMeta('אזור', route.area)}${detailMeta(route.catalogue_version === config.version ? 'משך כולל מהמרכז' : 'משך ליבת הטיול', route.duration)}${detailMeta('תחילת ליבת הטיול', route.start)}${detailMeta('סיום הליבה', route.end)}
      ${detailMeta('כבישים', route.roads)}${detailMeta('עונה', route.best)}${detailMeta('נבדק', route.checked_on)}${detailMeta('מבנה', routePatternLabel(route))}
    </div>
