...
(() => {
  ...
  function renderMusicBox(route) {
    const playlists = window.ROAD_BOOK_PLAYLISTS_CATALOG || [];
    const music = route.music_playlist || {};
    const primaryId = music.primary_playlist_id || (playlists[0] ? playlists[0].id : 'pl_israel_rock');
    
    const optionsHtml = playlists.map(p => {
      const selected = p.id === primaryId ? ' selected' : '';
      return `<option value="${escapeHtml(p.id)}"${selected}>${escapeHtml(p.name)} (${escapeHtml(p.genre)})</option>`;
    }).join('');

    const tracksHtml = (music.recommended_artists_and_tracks || []).map(t => 
      `<span class="track-tag" style="display:inline-block; background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); padding:4px 8px; border-radius:12px; font-size:12px; margin:2px; color:#cbd5e1;">${escapeHtml(t)}</span>`
    ).join(' ');
    ...
    const customHtml = custom
      ? `<div class="custom-playlist-active" style="margin-top:10px; background:rgba(14,165,233,0.1); padding:8px 12px; border-radius:6px; border:1px dashed #0ea5e9;">
          <strong>🎵 פלייליסט אישי שנשמר לקבוצה:</strong> ${escapeHtml(custom.title || custom.url)}
          <div class="custom-playlist-actions" style="margin-top:6px;">
            <a class="button primary" href="${escapeHtml(custom.url)}" target="_blank" rel="noopener">▶ פתיחת הפלייליסט האישי</a>
            <button class="button ghost" type="button" data-custom-playlist-edit="${escapeHtml(route.id)}">עריכה / הסרה</button>
          </div>
         </div>`
      : `<button class="button light custom-playlist-btn" type="button" data-custom-playlist="${escapeHtml(route.id)}" style="margin-top:10px;">➕ הגדרת פלייליסט אישי לקבוצה (Spotify / YouTube / Apple Music)</button>`;

    return `<section class="detail-section music-box-section card" style="background:var(--surface-2, #1e293b); border:1px solid rgba(14, 165, 233, 0.4); border-radius:12px; padding:16px; margin:16px 0;">
      <div class="music-box-header" style="margin-bottom:12px;">
        <h3 style="margin:0 0 4px 0; color:var(--accent-1, #38bdf8); font-size:18px;">🎵 סאונדטרק ומוזיקה לרכיבה</h3>
        <p style="margin:0; font-size:13px; color:var(--text-2, #cbd5e1);">בחירת פלייליסט מותאם לרכיבה מתוך ${playlists.length} סגנונות מומלצים בדאטה-בייס, או חיבור פלייליסט אישי:</p>
      </div>
      ...
      <div id="musicBoxDetails" class="music-box-details" style="background:rgba(0,0,0,0.25); padding:12px; border-radius:8px; margin-bottom:12px;">
        <p id="musicDesc" class="music-desc" style="margin:0 0 10px 0; font-size:13px; line-height:1.5; color:#e2e8f0;">${escapeHtml(music.primary_description || 'פלייליסט מותאם לרכיבה בכביש פתוח.')}</p>
        
        <div class="music-buttons-row" style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:10px;">
          <a id="btnSpotify" class="button spotify-btn" href="${escapeHtml(music.primary_spotify_url)}" target="_blank" rel="noopener" style="background:#1db954!important; color:#fff!important; font-weight:700; border-radius:8px; padding:8px 14px; text-decoration:none;">🟢 פתח ב-Spotify</a>
          <a id="btnYtMusic" class="button ytmusic-btn" href="${escapeHtml(music.primary_youtube_music_url)}" target="_blank" rel="noopener" style="background:#ff0000!important; color:#fff!important; font-weight:700; border-radius:8px; padding:8px 14px; text-decoration:none;">🔴 פתח ב-YouTube Music</a>
          <a id="btnAppleMusic" class="button applemusic-btn" href="${escapeHtml(music.primary_applemusic_url || 'https://music.apple.com')}" target="_blank" rel="noopener" style="background:#fa243c!important; color:#fff!important; font-weight:700; border-radius:8px; padding:8px 14px; text-decoration:none;">🍏 פתח ב-Apple Music</a>
        </div>

        <div class="tracks-row" style="margin-top:8px;">
          <small style="display:block; margin-bottom:4px; color:var(--text-3, #94a3b8);">שירים ואמנים בולטים ברשימה זו:</small>
