...
(() => {
  ...
  function dateSortValue(value) {
    ...
    if (dotted) return Date.UTC(Number(dotted[3]), Number(dotted[2]) - 1, Number(dotted[1]));
  ...
  function openRoute(routeId) {
    const route = routeById.get(routeId);
    if (!route) return;
    if (!$('#routeDialog').open) routeReturnFocusElement = document.activeElement;
    ...
    $('#routeDialogContent').innerHTML = `${issueBlock}<div class="route-detail-hero">
      ...
      <div class="chips"><span class="chip">${escapeHtml(route.level)}</span><span class="chip">${escapeHtml(route.road_character)}</span><span class="chip">${escapeHtml(day.dayKmLabel)} מהמרכז</span><span class="chip">${escapeHtml(day.timeLabel)}</span>${(route.trip_types || []).map((item) => `<span class="chip">${escapeHtml(item)}</span>`).join('')}${(route.themes || []).map((item) => `<span class="chip">${escapeHtml(item)}</span>`).join('')}${personalBadge(route.id)}</div>
    </div>
  ...
  function closeInviteDialog() {
    ...
    const returnButton = returnRouteId
      ...
      : null;
    (returnButton || lastFocusedElement)?.focus?.();
  }
