...
(() => {
  ...
  let pendingInitialJourneyId = null;
  ...
  function journeyDayId(journeyId, dayIndex) {
    return `journey-${journeyId}-day-${String(dayIndex + 1).padStart(2, '0')}`;
  }

  function journeyLodgingForDay(journey, dayIndex) {
    const nightNumber = dayIndex + 1;
    if (nightNumber >= (journey.days || []).length) return [];
    return (journey.lodging || []).filter((item) => new RegExp(`לילה\\s*${nightNumber}(?:\\D|$)`).test(item.night || ''));
  }

  function referencedJourneyDay(journey, routeId, dayIndex, dayCount) {
    const base = legacyById.get(routeId);
    if (!base) return null;
    return {
      ...base,
      id: journeyDayId(journey.id, dayIndex),
      index: dayIndex,
      title: base.title,
      connections: [],
      is_journey_day: true,
      ...
      journey_day_count: dayCount,
      journey_kind: 'referenced',
      journey_source_route_id: base.id,
      journey_story: journey.note || '',
      journey_food: '',
      journey_sleep: dayIndex < dayCount - 1 ? journey.sleep || '' : '',
      journey_lodging: [],
    };
  }

  function documentedJourneyDay(journey, day, dayIndex, dayCount) {
    const points = orderedRoutePoints(day.points || []);
    const raw = {
      id: journeyDayId(journey.id, dayIndex),
      title: day.title || `יום ${dayIndex + 1} — ${journey.title}`,
      region: 'מסע רב־יומי',
      area: journey.title,
      duration: 'יום רכיבה במסע — משך לחישוב במפה',
      km: day.km || 'לא צוין',
      km_num: null,
      core_km_num: null,
      style: day.roads || 'משולב',
      level: journey.level || 'בינוני',
      start: points[0] || journey.start || '',
      end: points.at(-1) || journey.end || '',
      roads: day.roads || 'לא צוינו',
      best: journey.best || 'לא צוין',
      summary: day.story || journey.story || '',
      story_big: day.story || journey.story || '',
      cautions: day.cautions || 'לא צוינו דגשים ייחודיים ליום; יש לבצע בדיקה עדכנית לפני היציאה.',
      fuel: day.fuel || 'לא צוינה תכנית תדלוק ליום; יש לתכנן לפני היציאה.',
      stops: points.map((name) => ({
        name,
        navigation_name: name,
        kind: 'נקודת דרך בתכנית היום',
        minutes: 0,
        story_long: 'נקודת דרך שנשמרה בתכנית היום. סיפור היום המלא מופיע בראש הטיול; פרטי ביקור, גישה ושעות יש לבדוק לפני היציאה.',
        era: 'לא סופק פירוט נפרד לנקודה',
        fuel: false,
      })),
      map_points: points,
      sources: [],
      seasonal: true,
      community: false,
      themes: ['מסע רב־יומי', 'תכנית יום'],
      trip_types: ['יום במסע רב־יומי'],
      road_profile: { fast: 0, twisty: 0, local: 0, urban: 0, note: 'לא סופק פילוח אחוזים נפרד ליום זה.', roads: [] },
      food_options: [],
      springs: [],
      connections: [],
      checked_on: 'לא צוינה בדיקת יום נפרדת',
      verification_level: 'תכנית מסע מן הספר',
      verification_note: 'היום נשמר מתכנית המסע המקורית. נקודות הדרך, הסיפור, התדלוק, האוכל והאזהרות מוצגים כפי שתועדו; הוא לא עבר ביקורת שחרור עצמאית כתיק מסלול מן הקטלוג.',
      route_shape: points[0] && points[0] === points.at(-1) ? 'מעגלי' : 'נקודה לנקודה',
      route_pattern: points[0] && points[0] === points.at(-1) ? 'loop' : 'snake',
      road_character: day.roads || 'משולב',
      quality_checks: [],
      quality_score: null,
      quality_status: 'תכנית מסע מתועדת — נדרשת בדיקה לפני יציאה',
    };
    const normalized = normalizeRoute(raw, dayIndex);
    return {
      ...normalized,
      is_journey_day: true,
      journey_id: journey.id,
      ...
      journey_day_index: dayIndex,
      journey_day_count: dayCount,
      journey_kind: 'documented',
      journey_source_route_id: '',
      journey_story: journey.story || '',
      journey_food: day.food || '',
      journey_sleep: '',
      journey_lodging: journeyLodgingForDay(journey, dayIndex),
      release_audit_result: null,
      release_audit_status: 'יום מתועד במסע — לא עבר ביקורת מסלול נפרדת',
      release_has_issue: false,
      release_issue_reason: '',
      release_issue_severity: '',
      release_issue_severity_label: '',
      release_issue_severity_description: '',
      release_audited_on: 'לא בוצעה ביקורת יום נפרדת',
      assistant_ready: false,
      assistant_support: 'limited_route_scope',
      content_scope: 'המידע מגיע מתכנית המסע המקורית. לא צורפו מקורות ישירים נפרדים ליום זה; יש לאמת כל יעד ותנאי דרך לפני היציאה.',
    };
  }

  function buildJourneys() {
    const twoDay = (legacy.multiday || []).map((journey) => {
      const ids = (journey.days || journey.routes?.map((item) => item.id) || []).filter(Boolean);
      return {
        ...journey,
        kind: 'two-day',
        day_routes: ids.map((routeId, index) => referencedJourneyDay(journey, routeId, index, ids.length)).filter(Boolean),
      };
    });
    const grand = (legacy.grandTours || []).map((journey) => ({
      ...journey,
      kind: 'grand',
      day_routes: (journey.days || []).map((day, index) => documentedJourneyDay(journey, day, index, journey.days.length)),
    }));
    return [...twoDay, ...grand].map((journey) => {
      const ids = journey.day_routes.map((route) => route.id);
      return {
        ...journey,
        day_routes: journey.day_routes.map((route, index) => ({
          ...route,
          connections: [...new Set([
            ...(route.connections || []),
            ids[index - 1],
  ...
  const journeyDayRoutes = journeys.flatMap((journey) => journey.day_routes);
  const personalActionRoutes = [...actionRoutes, ...journeyDayRoutes];
  const routeById = new Map(personalActionRoutes.map((route) => [route.id, route]));
  ...
  function routeShareUrl(route) {
    ...
    url.hash = route.is_journey_day ? 'journeysView' : route.release_has_issue ? 'issuesView' : 'routesView';
    return url.href;
  }

  function journeyShareUrl(journey) {
    const url = new URL(location.href);
    url.search = '';
    url.searchParams.set('journey', journey.id);
    url.hash = 'journeysView';
    return url.href;
  }
  ...
  function defaultMeetings(route) {
    if (route.is_journey_day) {
      const primaryDepart = '08:00';
      return {
        primaryPlace: route.start || 'תחילת יום המסע',
  ...
  function getMeetings(route) {
    ...
    const isGrandJourney = String(route.id).startsWith('grand-') || String(route.id).startsWith('journey:grand_');
    const legacyKey = isGrandJourney ? 'roadTripGrandDeparturesV03' : 'roadTripDeparturesV03';
    const legacyId = String(route.id).replace(/^grand-/, '').replace(/^journey:/, '');
  ...
  function routeDayEstimate(route) {
    ...
    const dayKm = route.is_journey_day && Number.isFinite(statedKm)
      ? statedKm
      : hasAuditedFullDistance
      ? roundTo(statedKm, 1)
    ...
    return {
      dayKm,
      dayKmLabel: route.is_journey_day && route.km
        ? route.km
        : Number.isFinite(dayKm) ? `${hasAuditedFullDistance ? '' : 'כ־'}${dayKm} ק״מ` : route.km || 'לא צוין',
      totalMinutes,
      ...
      distanceBasis: route.is_journey_day
        ? 'נתוני יום המסע כפי שנשמרו בתכנית; מחשבים זמן ומרחק עדכניים במפה לפני היציאה'
        : hasAuditedFullDistance
  ...
  function navigationBundleText(route) {
    const meetings = getMeetings(route);
    const approachLabel = route.is_journey_day ? 'מפת גישה לתחילת היום' : 'מפת גישה מהמרכז';
    const lines = [
    ...
    lines.push(
      '',
      `${approachLabel}: ${approachMapsUrl(route, meetings)}`,
  ...
  function releaseBadge(route) {
    if (route.is_journey_day && route.journey_kind === 'documented') {
      return '<span class="chip verify-warn">יום מתכנית מסע · נדרשת בדיקה</span>';
    }
  ...
  function routeCard(route) {
    ...
    const dayDistanceLabel = route.is_journey_day ? 'מרחק היום' : 'יום מהמרכז';
    const dayDurationLabel = route.is_journey_day ? 'משך היום המשוער' : 'משך יום משוער';
    const meetingLabel = route.is_journey_day ? 'מפגש בתחילת היום' : 'מפגש ראשון';
    const routeNumber = route.is_journey_day ? `יום ${route.journey_day_index + 1}` : String(route.index + 1).padStart(3, '0');
    const startChip = route.is_journey_day
      ? `יום ${route.journey_day_index + 1}/${route.journey_day_count} · התחלה: ${route.start}`
      : `★ יציאה: ${meetings.primaryPlace}`;
    return `<article class="route-card${issue ? ` issue-route-card issue-severity-${escapeHtml(route.release_issue_severity)}` : ''}" data-route-id="${escapeHtml(route.id)}" data-verification="${escapeHtml(route.verification_level)}" data-release-state="${issue ? 'warning' : 'pass'}" data-freshness="${freshness.state}" aria-labelledby="route-title-${escapeHtml(route.id)}"${issue ? ` data-issue-severity="${escapeHtml(route.release_issue_severity)}"` : ''}>
      ...
      <div class="route-card-main">
        <div class="route-card-top">
          <div><span class="route-number">${escapeHtml(routeNumber)}</span><h3 id="route-title-${escapeHtml(route.id)}">${escapeHtml(route.title)}</h3></div>
        ...
        <div class="day-facts" title="${escapeHtml(day.distanceBasis)}">
          <div><small>${escapeHtml(dayDistanceLabel)}</small><strong>${escapeHtml(day.dayKmLabel)}</strong></div>
          <div><small>${escapeHtml(dayDurationLabel)}</small><strong>${escapeHtml(day.timeLabel)}</strong></div>
          <div><small>${escapeHtml(meetingLabel)}</small><strong>${escapeHtml(meetings.primaryPlace)}</strong></div>
          <div><small>בדיקה אחרונה</small><strong>${escapeHtml(day.checkedOn)}</strong></div>
        </div>
        <div class="chips">
          <span class="chip central-origin-chip">${escapeHtml(startChip)}</span>
          ${route.is_journey_day ? `<span class="chip">מסע: ${escapeHtml(route.journey_title)}</span>` : ''}
        ...
        <div class="route-actions route-card-actions">
          ...
          <a class="button light" href="${escapeHtml(mapsUrl(route))}" target="_blank" rel="noopener">${route.is_journey_day ? 'Google Maps — מסלול היום' : issue ? 'Google Maps מהמרכז — לבדיקה' : 'Google Maps — מסלול מלא מהמרכז'}</a>
  ...
  function renderFavorites() {
    ...
    const selected = personalActionRoutes.filter((route) => favorites.has(route.id)
    ...
    $('#plannerFavoriteCount').textContent = personalActionRoutes.filter((route) => favorites.has(route.id)).length;
    $('#plannerWantCount').textContent = personalActionRoutes.filter((route) => personal[route.id]?.status === 'want').length;
    $('#plannerRiddenCount').textContent = personalActionRoutes.filter((route) => personal[route.id]?.status === 'ridden').length;
    renderRecentRoutes();
    loadVisibleMaps();
  }
  ...
  function routeExportHtml(route) {
    ...
    const journeyContext = route.is_journey_day ? `<section><h2>מיקום היום בתוך המסע</h2><p><strong>יום ${route.journey_day_index + 1} מתוך ${route.journey_day_count}:</strong> ${escapeHtml(route.journey_title)}</p>${route.journey_food ? `<p><strong>אוכל שתועד ליום:</strong> ${escapeHtml(route.journey_food)}</p>` : ''}${route.journey_sleep ? `<p><strong>לינה מוצעת אחרי היום:</strong> ${escapeHtml(route.journey_sleep)}</p>` : ''}${(route.journey_lodging || []).length ? `<ul>${route.journey_lodging.map((item) => `<li><strong>${escapeHtml(item.name || '')}</strong> — ${escapeHtml(item.type || '')}. ${escapeHtml(item.note || '')}${safeHttpsUrl(item.url) ? ` · <a href="${escapeHtml(item.url)}">אתר רשמי</a>` : ''}</li>`).join('')}</ul>` : ''}</section>` : '';
    const scopeLabel = route.is_journey_day ? `יום ${route.journey_day_index + 1}/${route.journey_day_count} במסע` : 'יום מהמרכז';
    const meetingHeading = route.is_journey_day ? 'נקודות מפגש והצטרפות ליום' : 'נקודות מפגש והצטרפות';
    const meetingLabel = route.is_journey_day ? 'נקודת תחילת היום' : 'נקודת מרכז';
    const estimateLabel = route.is_journey_day ? 'נתוני היום' : 'אומדן יום מלא מהמרכז';
    const approachLabel = route.is_journey_day ? 'מפת גישה לתחילת היום' : 'מפת גישה מהמרכז';
    return `<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive,nosnippet"><title>${escapeHtml(route.title)} — גרסה ${escapeHtml(config.version)}</title><style>${exportStyles()}</style></head><body><header><p>ספר הטיולים של אילן · גרסה ${escapeHtml(config.version)}</p><h1>${escapeHtml(route.title)}</h1><p>${escapeHtml(route.area)} · ${escapeHtml(scopeLabel)} · ${escapeHtml(day.dayKmLabel)} · ${escapeHtml(day.timeLabel)}</p></header><main>${issueBlock}${journeyContext}<div class="card no-print"><button onclick="window.print()">הדפסה / שמירה ל־PDF</button> <a class="button" href="${escapeHtml(routeShareUrl(route))}">פתיחה בספר</a></div><section class="meeting"><h2>${escapeHtml(meetingHeading)}</h2><p><strong>${escapeHtml(meetingLabel)}:</strong> ${escapeHtml(meetings.primaryPlace)} — מפגש ${escapeHtml(meetings.primaryMeet)}, יציאה ${escapeHtml(meetings.primaryDepart)} <a class="button" href="${escapeHtml(wazeUrl(meetings.primaryPlace))}">Waze</a></p>${secondary}<p><strong>תחילת מסלול הטיול:</strong> ${escapeHtml(route.start)}</p><p><strong>${escapeHtml(estimateLabel)}:</strong> ${escapeHtml(day.dayKmLabel)} · ${escapeHtml(day.timeLabel)}.</p><p class="muted">${escapeHtml(day.distanceBasis)}. השעות והמרחק הם כלי תכנון; בודקים ב־Google Maps ומעדכנים לפני הפצה.</p><a class="button" href="${escapeHtml(approachMapsUrl(route, meetings))}">${escapeHtml(approachLabel)}</a><a class="button" href="${escapeHtml(mapsUrl(route))}">מפת מסלול הטיול</a></section><div class="card grid"><div><b>התחלה</b><br>${escapeHtml(route.start)}</div><div><b>סיום</b><br>${escapeHtml(route.end)}</div><div><b>רמה</b><br>${escapeHtml(route.level)}</div><div><b>עונה</b><br>${escapeHtml(route.best)}</div><div><b>כבישים</b><br>${escapeHtml(route.roads)}</div><div><b>אופי</b><br>${escapeHtml(route.road_character)}</div><div><b>סוגי טיול</b><br>${escapeHtml((route.trip_types || []).join(' · '))}</div><div><b>אימות</b><br>${escapeHtml(route.verification_level)}</div></div><section><h2>סיפור הדרך</h2><p>${escapeHtml(route.story_big || route.summary).replace(/\n/g, '<br>')}</p></section><section><h2>מפת המסלול</h2><iframe class="map" loading="lazy" src="${escapeHtml(embedUrl(route))}"></iframe></section><section><h2>פרופיל כביש</h2><div class="meters"><div><b>${Number(profile.fast) || 0}%</b>מהיר</div><div><b>${Number(profile.twisty) || 0}%</b>מפותל</div><div><b>${Number(profile.local) || 0}%</b>אזורי</div><div><b>${Number(profile.urban) || 0}%</b>עירוני</div>${gravelMeter}</div><p>${escapeHtml(profile.note || 'טרם הושלם פרופיל כביש.')}</p></section><h2>התחנות וסיפורי הדרך</h2>${stops}${springs ? `<section><h2>מים ומעיינות</h2><ul>${springs}</ul></section>` : ''}${food ? `<section><h2>קפה ואוכל</h2><ul>${food}</ul></section>` : ''}${route.journey_food ? `<section><h2>אוכל שתועד ליום</h2><p>${escapeHtml(route.journey_food)}</p></section>` : ''}<section><h2>תדלוק</h2><p>${escapeHtml(route.fuel)}</p></section><section class="warn"><h2>דגשים ובטיחות</h2><p>${escapeHtml(route.cautions)}</p></section>${connections ? `<section><h2>המשך טבעי למסלולים נוספים</h2><ul>${connections}</ul></section>` : ''}<section><h2>מקורות</h2><p>${sources || 'לא צורפו מקורות תקינים.'}</p></section><section class="warn"><h2>אחריות הרוכב</h2><p>זהו טיול חברים לא־מאורגן. כל רוכב רוכב באחריותו הבלעדית ואחראי לרישיון ולביטוח תקפים, למיגון, לתקינות האופנוע, לשירותי גרירה ולציות לחוק. בודקים את הדרך ואת המקורות הרשמיים ביום היציאה.</p></section></main></body></html>`;
  }
  ...
  function journeyTarget(journey) {
    const first = journey.day_routes[0];
    const last = journey.day_routes.at(-1);
    return {
      id: `journey:${journey.id}`,
      title: journey.title,
      region: 'מסע רב־יומי',
      area: journey.best || journey.sleep || '',
      start: journey.start || first?.start || '',
      end: journey.end || last?.end || '',
      stops: journey.day_routes.map((route, index) => ({ name: `יום ${index + 1}: ${route.title}`, kind: route.start })),
      story_big: journey.story || journey.note || '',
      summary: journey.story || journey.note || '',
      km: journey.km || journey.day_routes.map((route) => route.km).join(' + '),
      level: journey.level || 'לפי ימי המסע',
      best: journey.best || 'לפי ימי המסע',
    };
  }

  function routeExportMainHtml(route) {
    const html = routeExportHtml(route);
    return html.slice(html.indexOf('<main>') + 6, html.lastIndexOf('</main>'));
  }

  function journeyExportHtml(journey) {
    const target = journeyTarget(journey);
    ...
    const days = journey.day_routes.map((route, index) => `<section class="journey-export-day"><h2>יום ${index + 1} מתוך ${journey.day_routes.length}: ${escapeHtml(route.title)}</h2><p class="muted">זהו תיק יום מלא מתוך המסע. אפשר להשתמש בקישורים ובכלי התכנון של היום בנפרד.</p>${routeExportMainHtml(route)}</section>`).join('');
    ...
    const simpleSleep = journey.sleep ? `<section><h2>לינה מוצעת מן הספר</h2><p>${escapeHtml(journey.sleep)}</p><a class="button" href="${escapeHtml(googleSearchUrl(`לינה ${journey.sleep}`))}">חיפוש לינה במפה</a></section>` : '';
    return `<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive,nosnippet"><title>${escapeHtml(journey.title)} — גרסה ${escapeHtml(config.version)}</title><style>${exportStyles()}.journey-export-day{border:3px solid #176979}.journey-export-day>h2{background:#176979;color:#fff;margin:-17px -17px 14px;padding:14px}.journey-export-day .no-print{display:block}</style></head><body><header><p>ספר הטיולים של אילן · גרסה ${escapeHtml(config.version)}</p><h1>${escapeHtml(journey.title)}</h1><p>${escapeHtml(journey.km || '')} · ${journey.day_routes.length} ימים מלאים</p></header><main><div class="card no-print"><button onclick="window.print()">הדפסה / שמירה ל־PDF</button> <a class="button" href="${escapeHtml(journeyShareUrl(journey))}">פתיחת המסע בספר</a></div><section class="meeting"><h2>יציאת המסע</h2><p><strong>נקודת מרכז:</strong> ${escapeHtml(meetings.primaryPlace)} — מפגש ${escapeHtml(meetings.primaryMeet)}, יציאה ${escapeHtml(meetings.primaryDepart)}</p>${second}<p class="muted">לכל יום בהמשך יש נקודת התחלה ושעות הניתנות לעריכה בנפרד.</p></section><section><h2>על המסע</h2><p>${escapeHtml(journey.story || journey.note || '')}</p></section>${simpleSleep}${lodging ? `<section><h2>אפשרויות לינה</h2><ul>${lodging}</ul></section>` : ''}${days}<section class="warn"><p>זהו טיול חברים לא־מאורגן. כל רוכב רוכב באחריותו הבלעדית ואחראי לרישיון, לביטוח, למיגון, לתקינות האופנוע ולשירותי גרירה. בודקים כל יום וכל יעד מחדש לפני היציאה.</p></section></main></body></html>`;
  }

  function exportJourney(journeyId) {
    const journey = journeyById.get(journeyId);
    if (journey) downloadHtml(`${journey.title}-כל-הימים-גרסה-${config.version}.html`, journeyExportHtml(journey));
  }

  function journeyNavigationText(journey) {
    return [`ספר הטיולים של אילן — ניווט מלא למסע`, `מסע: ${journey.title}`, `${journey.day_routes.length} ימים · גרסה ${config.version}`, '', ...journey.day_routes.flatMap((route, index) => [
      `========== יום ${index + 1}/${journey.day_routes.length} ==========`,
      navigationBundleText(route),
      '',
    ])].join('\n');
  }

  function journeyShareText(journey) {
    return [`🏍️ מסע רב־יומי: ${journey.title}`, `${journey.day_routes.length} ימים`, journey.story || journey.note || '', '', ...journey.day_routes.map((route, index) => `${index + 1}. ${route.title}\n${route.start} ← ${route.end} · ${route.km}\n${mapsUrl(route)}`), '', `פתיחת המסע בספר: ${journeyShareUrl(journey)}`, '', 'כל יום הוא תיק טיול נפרד. בודקים מסלול, מזג אוויר, חסימות, תדלוק ולינה לפני היציאה.'].join('\n');
  }

  function journeyAiPrompt(journey) {
    return `אני מתכנן מסע אופנועים רב־יומי בישראל בשם „${journey.title}”. להלן תכנית קיימת מתוך ספר הטיולים של אילן:\n\n${journeyShareText(journey)}\n\nנתח כל יום בנפרד באותו מבנה: רצף דרך, זמן ומרחק עדכניים, עצירות, תדלוק, אוכל, לינה, סיכונים וחלופות. אמת כל נתון מול מקורות רשמיים ועדכניים, צרף קישורים ותאריך בדיקה, וסמן בבירור מידע חסר או סותר. אל תמציא עובדות, שעות, מצב דרך או סטטוס פתיחה. בסוף בדוק גם שהחיבור בין סוף כל יום לתחילת היום הבא הגיוני.`;
  }

  function openJourneyMaps(journey) {
    journey.day_routes.forEach((route, index) => setTimeout(() => window.open(mapsUrl(route), '_blank', 'noopener'), index * 350));
  }

  function journeyCard(journey) {
    const target = journeyTarget(journey);
    const meetings = getMeetings(target);
    const dayRows = journey.day_routes.map((route, index) => `<section class="journey-day-full" aria-labelledby="journey-${escapeHtml(journey.id)}-day-${index + 1}"><div class="journey-day-separator"><span>יום ${index + 1} מתוך ${journey.day_routes.length}</span><h5 id="journey-${escapeHtml(journey.id)}-day-${index + 1}">${escapeHtml(route.title)}</h5></div>${routeCard(route)}</section>`).join('');
    ...
    return `<section class="journey-card journey-package" data-journey-id="${escapeHtml(journey.id)}">
      <span class="eyebrow">מארז של ${journey.day_routes.length} טיולים יומיים מלאים${journey.km ? ` · ${escapeHtml(journey.km)}` : ''}</span>
      <h4>${escapeHtml(journey.title)}</h4>
      <p>${escapeHtml(journey.story || journey.note || '')}</p>
      <div class="chips">${journey.level ? `<span class="chip">${escapeHtml(journey.level)}</span>` : ''}${journey.best ? `<span class="chip">${escapeHtml(journey.best)}</span>` : ''}<span class="chip">${journey.day_routes.length} ימים נפרדים</span></div>
      ${journey.sleep ? `<p class="journey-note"><strong>לינה מוצעת:</strong> ${escapeHtml(journey.sleep)}</p>` : ''}
      <div class="meeting-summary"><strong>יציאת המסע:</strong> ${escapeHtml(meetings.primaryPlace)} · ${escapeHtml(meetings.primaryDepart)}<br><small>בתוך כל יום אפשר לערוך בנפרד את נקודת ההתחלה, ההצטרפות והשעות.</small></div>
      <div class="route-actions journey-actions">
        <button class="button primary" type="button" data-invite-journey="${escapeHtml(journey.id)}">הזמנה ולוח שנה לכל המסע</button>
        <button class="button accent" type="button" data-export-journey="${escapeHtml(journey.id)}">ייצוא HTML מלא — כל הימים</button>
        <button class="button ghost" type="button" data-copy-journey-navigation="${escapeHtml(journey.id)}">העתקת ניווט לכל הימים</button>
        <button class="button ghost" type="button" data-open-journey-maps="${escapeHtml(journey.id)}">פתיחת מפות כל הימים</button>
        <button class="button ghost" type="button" data-copy-journey-summary="${escapeHtml(journey.id)}">העתקת תקציר המסע</button>
        <button class="button ghost" type="button" data-copy-journey-link="${escapeHtml(journey.id)}">העתקת קישור למסע</button>
        <button class="button ghost" type="button" data-copy-journey-ai="${escapeHtml(journey.id)}" title="${escapeHtml(AI_COPY_TOOLTIP)}">העתקת המסע ל־AI</button>
      </div>
      <details class="journey-details"><summary>פתיחת ${journey.day_routes.length} הטיולים היומיים וכל הכלים שלהם</summary><div class="journey-days">${dayRows}</div>${lodgingRows ? `<section class="journey-lodging"><h5>אפשרויות לינה מן הספר</h5><ul class="lodging-list">${lodgingRows}</ul></section>` : ''}</details>
    </section>`;
  }

  function renderJourneys() {
    const openJourneyIds = new Set($$('#journeysView .journey-details[open]').map((details) => details.closest('[data-journey-id]')?.dataset.journeyId).filter(Boolean));
    const twoDay = journeys.filter((journey) => journey.kind === 'two-day');
    const grand = journeys.filter((journey) => journey.kind === 'grand');
    $('#multiDayCount').textContent = twoDay.length;
    $('#grandTourCount').textContent = grand.length;
    $('#multiDayGrid').innerHTML = twoDay.map(journeyCard).join('');
    $('#grandTourGrid').innerHTML = grand.map(journeyCard).join('');
    openJourneyIds.forEach((journeyId) => {
      const details = document.querySelector(`[data-journey-id="${CSS.escape(journeyId)}"] .journey-details`);
  ...
  function openRoute(routeId) {
    ...
    const documentedJourneyDay = route.is_journey_day && route.journey_kind === 'documented';
    const journeyContext = route.is_journey_day ? `<section class="detail-section journey-day-context"><span class="eyebrow">יום ${route.journey_day_index + 1} מתוך ${route.journey_day_count}</span><h3>${escapeHtml(route.journey_title)}</h3><p>היום הזה הוא טיול עצמאי בתוך המסע. כל שינוי בנקודות המפגש, בתאריך, בתכנון האישי או בייצוא נשמר ליום הזה בלבד.</p>${route.journey_source_route_id ? `<p><strong>מבוסס על תיק המסלול המלא:</strong> ${escapeHtml(route.journey_source_route_id)}</p>` : '<p><strong>מקור התוכן:</strong> תכנית היום שנשמרה במסע המקורי; אין תיק ביקורת נפרד ליום זה.</p>'}${route.journey_food ? `<p><strong>אוכל שתועד ליום:</strong> ${escapeHtml(route.journey_food)}</p>` : ''}${route.journey_sleep ? `<p><strong>לינה מוצעת אחרי היום:</strong> ${escapeHtml(route.journey_sleep)}</p>` : ''}${(route.journey_lodging || []).length ? `<div class="info-grid">${route.journey_lodging.map((item) => `<article class="info-card"><h4>${escapeHtml(item.name || '')}</h4><p>${escapeHtml(item.type || '')}. ${escapeHtml(item.note || '')}</p>${safeHttpsUrl(item.url) ? `<a class="button ghost" href="${escapeHtml(item.url)}" target="_blank" rel="noopener">אתר רשמי</a>` : ''}</article>`).join('')}</div>` : ''}</section>` : '';
    const qualityPanel = documentedJourneyDay
      ? `<section class="detail-section quality-panel quality-panel-warning"><div class="quality-head"><div><h3>מעמד תיק היום</h3><strong>תכנית מסע מתועדת — לא ביקורת PASS</strong></div><div class="quality-score">לבדיקה</div></div><p>${escapeHtml(route.verification_note || '')}</p><p><strong>מה כן קיים:</strong> סדר נקודות, סיפור יום, כבישים, תדלוק, אוכל ואזהרות כפי שנשמרו במסע. <strong>מה אינו נטען:</strong> לא הומצאו מקורות, אחוזי כביש, שעות פתיחה או בדיקת דרך שלא קיימים בתיק.</p></section>`
    ...
    const dayDistanceContext = route.is_journey_day ? `${day.dayKmLabel} ביום המסע` : `${day.dayKmLabel} מהמרכז`;
    const totalDayLabel = route.is_journey_day ? 'נתוני היום במסע' : 'יום כולל מהמרכז';
    const meetingOriginLabel = route.is_journey_day ? 'תחילת היום' : 'מרכז';
    const approachMapLabel = route.is_journey_day ? 'מפת גישה לתחילת היום' : 'מפת גישה מהמרכז';
    const fullMapLabel = route.is_journey_day ? 'מסלול היום ב־Google Maps' : releaseIssue ? 'מסלול מלא מהמרכז — לבדיקה' : 'מסלול מלא מהמרכז ב־Google Maps';
    $('#routeDialogContent').innerHTML = `${issueBlock}<div class="route-detail-hero">
      <span class="eyebrow">${route.is_journey_day ? `יום ${route.journey_day_index + 1}/${route.journey_day_count} · ` : ''}${escapeHtml(route.region)} · ${escapeHtml(route.verification_level)}</span>
      <h2 id="routeDialogTitle">${escapeHtml(route.title)}</h2>
      <p>${escapeHtml(route.story_big || route.summary)}</p>
      <div class="chips">${freshnessBadge(route)}<span class="chip">${escapeHtml(route.level)}</span><span class="chip">${escapeHtml(route.road_character)}</span><span class="chip">${escapeHtml(dayDistanceContext)}</span><span class="chip">${escapeHtml(day.timeLabel)}</span>${(route.trip_types || []).map((item) => `<span class="chip">${escapeHtml(item)}</span>`).join('')}${(route.themes || []).map((item) => `<span class="chip">${escapeHtml(item)}</span>`).join('')}${personalBadge(route.id)}</div>
    </div>
    ${journeyContext}
    <div class="detail-grid">
      ${detailMeta('אזור', route.area)}${detailMeta(totalDayLabel, `${day.dayKmLabel} · ${day.timeLabel}`)}${detailMeta('תחילת ליבת הטיול', route.start)}${detailMeta('סיום הליבה', route.end)}
    ...
    ${route.journey_food ? `<section class="detail-section"><h3>אוכל שתועד ליום</h3><p>${escapeHtml(route.journey_food)}</p></section>` : ''}
  ...
  function resolveInviteTarget(targetId) {
    if (String(targetId).startsWith('journey:') || String(targetId).startsWith('grand:')) {
      const journeyId = String(targetId).replace(/^(?:journey|grand):/, '');
      const journey = journeyById.get(journeyId);
      return journey ? { route: journeyTarget(journey), journey } : null;
    }
  ...
  function ensureInviteOptions(extraTarget = null) {
    ...
    if (extraTarget?.journey) {
      ...
      option.value = `journey:${extraTarget.journey.id}`;
      option.textContent = `מסע: ${extraTarget.journey.title}`;
      select.appendChild(option);
    } else if (extraTarget?.route && !select.querySelector(`option[value="${CSS.escape(extraTarget.route.id)}"]`)) {
      const option = document.createElement('option');
      option.value = extraTarget.route.id;
      option.textContent = extraTarget.route.is_journey_day
        ? `יום ${extraTarget.route.journey_day_index + 1}: ${extraTarget.route.title}`
        : extraTarget.route.title;
      select.appendChild(option);
    }
  }
  ...
  function updateCalendarStatus() {
    ...
    $('#calendarStatus').textContent = $('#inviteDate').value
      ? target.journey
        ? `תאריך ההתחלה נקבע. קובץ היומן יכלול ${target.journey.day_routes.length} אירועים — אחד לכל יום.`
        : 'התאריך נקבע. אפשר להוריד קובץ יומן עם שעות המפגש והיציאה.'
      : target.journey ? 'בחרו תאריך התחלה כדי להוסיף ליומן את כל ימי המסע.' : 'בחרו תאריך כדי לאפשר הוספה ליומן.';
  }

  function calendarDateValue(baseDate, offsetDays) {
    const date = new Date(baseDate.getFullYear(), baseDate.getMonth(), baseDate.getDate() + offsetDays, 12, 0, 0, 0);
    return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
  }

  function calendarEventLines(route, dateValue, meetings, summaryPrefix = '') {
    ...
    if (!startsAt || !departureAt || !Number.isFinite(estimate.totalMinutes)) return null;
    ...
    return [
      ...
      `SUMMARY:${icsEscape(`${summaryPrefix}טיול אופנועים — ${route.title}`)}`,
  ...
  function exportInviteCalendar() {
    ...
    const baseDate = localCalendarDateTime(dateValue, '12:00');
    if (!baseDate) {
      status.textContent = 'לא ניתן לייצא ליומן: תאריך ההתחלה אינו תקין.';
      return;
    }
    const calendarRoutes = target.journey ? target.journey.day_routes : [target.route];
    const firstMeetings = readMeetingsFromForm(target.route);
    const events = [];
    for (const [index, route] of calendarRoutes.entries()) {
      const eventDate = calendarDateValue(baseDate, index);
      const meetings = target.journey && index > 0 ? getMeetings(route) : { ...getMeetings(route), ...firstMeetings };
      const eventLines = calendarEventLines(route, eventDate, meetings, target.journey ? `מסע ${target.journey.title} · יום ${index + 1}/${calendarRoutes.length} — ` : '');
      if (!eventLines) {
        status.textContent = `לא ניתן לייצא ליומן: התאריך, השעה או משך יום ${index + 1} אינם תקינים.`;
        return;
      }
    ...
    const lines = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Ilan Road Book//PWA 2.6.0//HE',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH',
      ...events,
      'END:VCALENDAR',
    ...
    const exportTitle = target.journey ? `${target.journey.title}-כל-הימים` : target.route.title;
    downloadTextFile(`${exportTitle}-2.6.0.ics`, lines.map(foldIcsLine).join('\r\n'), 'text/calendar;charset=utf-8');
    status.textContent = target.journey
      ? `קובץ היומן הורד עם ${calendarRoutes.length} אירועים — אירוע נפרד לכל יום במסע.`
      : 'קובץ היומן הורד. פתחו אותו כדי להוסיף את הטיול ליומן שבמכשיר.';
  }
  ...
  function openInvite(targetId, journey = null) {
    const target = journey ? { route: journeyTarget(journey), journey } : resolveInviteTarget(targetId);
    if (!target) return;
    const key = journey ? `journey:${journey.id}` : targetId;
  ...
  function recalculateMeetings() {
    ...
    const minutes = Number.isFinite(Number(current.estimatedMinutes))
      ? Number(current.estimatedMinutes)
      : meetingPreset(target.route).minutes;
...
הפרד בבירור בין עובדות מאומתות, מסקנות והמלצות. בטיחות, חוק והנחיות רשמיות עדכניות קודמים לתכנית הטיול.`;
  ...
  function requestedJourneyFromAddress() {
    try {
      const journeyId = new URL(location.href).searchParams.get('journey');
      return journeyId && journeyById.has(journeyId) ? journeyId : null;
    } catch {
      return null;
    }
  }

  function clearJourneyAddress() {
    try {
      const url = new URL(location.href);
      if (!url.searchParams.has('journey')) return;
      url.searchParams.delete('journey');
      history.replaceState(null, '', url.href);
    } catch {
      // כתובת מקומית לא תקינה אינה מונעת שימוש במסעות.
    }
  }
  ...
  function openPendingInitialJourney() {
    const journeyId = pendingInitialJourneyId;
    pendingInitialJourneyId = null;
    if (!journeyId || !journeyById.has(journeyId)) return;
    const card = document.querySelector(`[data-journey-id="${CSS.escape(journeyId)}"]`);
  ...
  function bindEvents() {
    ...
    $('#journeysView').addEventListener('toggle', (event) => {
      if (event.target.matches?.('.journey-details') && event.target.open) loadVisibleMaps();
    }, true);
    ...
    document.addEventListener('click', (event) => {
      ...
      const inviteJourney = event.target.closest('[data-invite-journey]');
      if (inviteJourney) {
        const journey = journeyById.get(inviteJourney.dataset.inviteJourney);
        if (journey) openInvite('', journey);
      }
      const exportJourneyButton = event.target.closest('[data-export-journey]');
      if (exportJourneyButton) exportJourney(exportJourneyButton.dataset.exportJourney);
      const copyJourneyNavigation = event.target.closest('[data-copy-journey-navigation]');
      if (copyJourneyNavigation) {
        const journey = journeyById.get(copyJourneyNavigation.dataset.copyJourneyNavigation);
        if (journey) void copyWithFeedback(copyJourneyNavigation, journeyNavigationText(journey), 'העתקת ניווט לכל הימים', 'כל ניווט המסע הועתק ✓', 'הניווט הועתק לפי ימים. מדביקים ושומרים את ההפרדה בין הימים.');
      }
      const openJourneyMapsButton = event.target.closest('[data-open-journey-maps]');
      if (openJourneyMapsButton) {
        const journey = journeyById.get(openJourneyMapsButton.dataset.openJourneyMaps);
        if (journey) openJourneyMaps(journey);
      }
      const copyJourneySummary = event.target.closest('[data-copy-journey-summary]');
      if (copyJourneySummary) {
        const journey = journeyById.get(copyJourneySummary.dataset.copyJourneySummary);
        if (journey) void copyWithFeedback(copyJourneySummary, journeyShareText(journey), 'העתקת תקציר המסע', 'תקציר המסע הועתק ✓');
      }
      const copyJourneyLink = event.target.closest('[data-copy-journey-link]');
      if (copyJourneyLink) {
        const journey = journeyById.get(copyJourneyLink.dataset.copyJourneyLink);
        if (journey) void copyWithFeedback(copyJourneyLink, journeyShareUrl(journey), 'העתקת קישור למסע', 'קישור המסע הועתק ✓');
      }
      const copyJourneyAi = event.target.closest('[data-copy-journey-ai]');
      if (copyJourneyAi) {
        const journey = journeyById.get(copyJourneyAi.dataset.copyJourneyAi);
        if (journey) void copyWithFeedback(copyJourneyAi, journeyAiPrompt(journey), 'העתקת המסע ל־AI', 'הועתק ✓ · עכשיו להדביק ב־AI', AI_COPY_SUCCESS_HELP);
      }
    ...
    $('#exportInviteRoute').addEventListener('click', () => {
      ...
      if (target.journey) exportJourney(target.journey.id);
  ...
  function init() {
    ...
    const requestedJourneyId = requestedJourneyFromAddress();
    ...
    if (requestedJourneyId) pendingInitialJourneyId = requestedJourneyId;
    else {
      try {
        if (new URL(location.href).searchParams.has('journey')) clearJourneyAddress();
      } catch {
        // כתובת לא תקינה אינה מונעת את פתיחת הספר.
      }
    }
    ...
    const initialView = requestedRoute
      ? requestedRoute.is_journey_day ? 'journeysView' : requestedRoute.release_has_issue ? 'issuesView' : 'routesView'
      : requestedJourneyId ? 'journeysView'
