/**
 * בדיקת דפדפן מקצה לקצה — גרסת מסמך 2.6.0
 * גרסת מוצר: 2.6.0
 * גרסת קטלוג מסלולים: 2.3.0
 *
 * הרצה: הגדרת NODE_PATH לספריות סביבת Codex ואז:
 * node tests/browser-qa-v2.6.0.cjs http://127.0.0.1:4182/ --write
 */
...
async function verifyChecklist(page) {
  ...
  const unrelatedValue = JSON.stringify({ untouched: true, marker: 'qa-2.6.0' });
...
async function verifyJourneys(page) {
  await page.locator('[data-view="journeysView"]').first().click();
  assert.equal((await page.locator('#multiDayCount').textContent()).trim(), '18');
  assert.equal((await page.locator('#grandTourCount').textContent()).trim(), '3');
  assert.equal(await page.locator('[data-journey-id]').count(), 21);

  const twoDay = page.locator('[data-journey-id="m01"]');
  await twoDay.locator('.journey-details > summary').click();
  assert.equal(await twoDay.locator('.journey-day-full').count(), 2);
  assert.equal(await twoDay.locator('.journey-day-full > .route-card').count(), 2);
  const dayIds = await twoDay.locator('.journey-day-full > .route-card').evaluateAll((nodes) => nodes.map((node) => node.dataset.routeId));
  assert.equal(new Set(dayIds).size, 2);
  assert.ok(dayIds.every((id) => /^journey-m01-day-0[12]$/.test(id)), dayIds.join(','));
  for (const selector of [
    '[data-open-route]', '[data-ready-share]', '[data-copy-navigation]', '[data-copy-route-link]',
    '[data-add-combined]', '[data-invite]', '[data-export-route]',
  ]) assert.ok(await twoDay.locator(selector).count() >= 2, selector);
  assert.ok(await twoDay.locator('[data-ai-route]').count() >= 1, 'AI remains available on journey days whose regular-route quality status permits it');

  await twoDay.locator(`[data-open-route="${dayIds[0]}"]`).first().click();
  await page.locator('#routeDialog').waitFor({ state: 'visible' });
  assert.match(await page.locator('#routeDialogContent').textContent(), /יום 1 מתוך 2/);
  assert.match(await page.locator('#routeDialogContent').textContent(), /כל שינוי בנקודות המפגש.*נשמר ליום הזה בלבד/);
  assert.ok(await page.locator('#routeDialog .stop-card').count() > 0);
  assert.equal(await page.locator(`#routeDialog [data-copy-navigation="${dayIds[0]}"]`).count(), 1);
  assert.ok(await page.locator(`#routeDialog [data-invite="${dayIds[0]}"]`).count() >= 1);
  const dayExport = await downloadedHtml(page, () => page.locator(`#routeDialog [data-export-route="${dayIds[0]}"]`).click());
  assert.match(dayExport.filename, /2\.6\.0.*\.html$/);
  assert.match(dayExport.html, /יום 1 מתוך 2/);
  assert.match(dayExport.html, /נקודות מפגש והצטרפות ליום/);
  assert.match(dayExport.html, /התחנות וסיפורי הדרך/);
  assert.match(dayExport.html, /[?&amp;]route=journey-m01-day-01|[?&]route=journey-m01-day-01/);
  await page.locator('[data-close-dialog]').click();

  const wholeExport = await downloadedHtml(page, () => twoDay.locator('[data-export-journey]').click());
  assert.match(wholeExport.filename, /כל-הימים.*2\.6\.0.*\.html$/);
  assert.equal((wholeExport.html.match(/class="journey-export-day"/g) || []).length, 2);
  assert.match(wholeExport.html, /יום 1 מתוך 2/);
  assert.match(wholeExport.html, /יום 2 מתוך 2/);
  assert.match(wholeExport.html, /פתיחת המסע בספר/);

  await twoDay.locator('[data-copy-journey-navigation]').click();
  const journeyNavigation = await page.evaluate(() => navigator.clipboard.readText());
  assert.match(journeyNavigation, /ניווט מלא למסע/);
  assert.match(journeyNavigation, /יום 1\/2/);
  assert.match(journeyNavigation, /יום 2\/2/);
  assert.match(journeyNavigation, /journey-m01-day-01/);
  assert.match(await twoDay.locator('[data-copy-journey-ai]').getAttribute('title'), /Clipboard.*Paste|לוח.*הדבקה/);

  await twoDay.locator('[data-invite-journey]').click();
  await page.locator('#inviteDialog').waitFor({ state: 'visible' });
  assert.match(await page.locator('#invitePreview').inputValue(), /מפות כל ימי המסע/);
  await page.locator('#inviteDate').fill('2026-10-10');
  assert.match(await page.locator('#calendarStatus').textContent(), /2 אירועים/);
  const journeyCalendar = await downloadedFile(page, () => page.locator('#exportInviteCalendar').click());
  const unfoldedJourneyCalendar = journeyCalendar.text.replace(/\r\n[ \t]/g, '');
  assert.equal((unfoldedJourneyCalendar.match(/BEGIN:VEVENT\r\n/g) || []).length, 2);
  assert.match(unfoldedJourneyCalendar, /DTSTART;TZID=Asia\/Jerusalem:20261010T/);
  assert.match(unfoldedJourneyCalendar, /DTSTART;TZID=Asia\/Jerusalem:20261011T/);
  assert.match(unfoldedJourneyCalendar, /route=journey-m01-day-01/);
  assert.match(unfoldedJourneyCalendar, /route=journey-m01-day-02/);
  await page.locator('[data-close-invite]').click();

  const grand = page.locator('[data-journey-id="grand_south_4"]');
  await grand.locator('.journey-details > summary').click();
  assert.equal(await grand.locator('.journey-day-full').count(), 4);
  const grandFirstId = await grand.locator('.journey-day-full > .route-card').first().getAttribute('data-route-id');
  assert.equal(grandFirstId, 'journey-grand_south_4-day-01');
  await grand.locator(`[data-open-route="${grandFirstId}"]`).first().click();
  await page.locator('#routeDialog').waitFor({ state: 'visible' });
  const documentedText = await page.locator('#routeDialogContent').textContent();
  assert.match(documentedText, /תכנית מסע מתועדת — לא ביקורת PASS/);
  assert.match(documentedText, /לא הומצאו מקורות/);
  assert.match(documentedText, /אוכל שתועד ליום/);
  await page.locator(`#routeDialog [data-copy-route-link="${grandFirstId}"]`).click();
  const dayLink = new URL(await page.evaluate(() => navigator.clipboard.readText()));
  assert.equal(dayLink.searchParams.get('route'), grandFirstId);
  assert.equal(dayLink.hash, '#journeysView');
  await page.locator('[data-close-dialog]').click();
  ...
  const overflow = await page.evaluate(() => ({
    document: document.documentElement.scrollWidth - document.documentElement.clientWidth,
    body: document.body.scrollWidth - document.body.clientWidth,
    cards_inside_viewport: [...document.querySelectorAll('#journeysView .journey-card, #journeysView .route-card')]
      .filter((node) => node.getClientRects().length)
      .every((node) => node.getBoundingClientRect().right <= innerWidth + 1 && node.getBoundingClientRect().left >= -1),
  }));
  assert.ok(overflow.document <= 1 && overflow.body <= 1 && overflow.cards_inside_viewport, JSON.stringify(overflow));
  await page.setViewportSize({ width: 1280, height: 900 });
  await page.locator('[data-view="routesView"]').first().click();
  return {
    journeys: 21,
    day_routes: 50,
    two_day_full_routes: 2,
    grand_documented_routes: 4,
    whole_html_days: 2,
    calendar_events: 2,
...
async function main() {
  ...
  assert.match(assets.title, /2\.6\.0/);
  assert.equal(assets.lang, 'he');
  assert.equal(assets.dir, 'rtl');
  assert.equal(assets.manifest, './manifest-2.6.0.webmanifest');
  assert.ok(assets.styles.includes('./assets/app-v2.6.0.css'));
  assert.ok(assets.scripts.includes('./data/config-v2.6.0.js'));
  assert.ok(assets.scripts.includes('./assets/app-v2.6.0.js'));
  ...
  const fetchedAssets = await page.evaluate(async () => {
    const [manifest, config, serviceWorker] = await Promise.all([
      fetch('./manifest-2.6.0.webmanifest').then((response) => response.json()),
      fetch('./data/config-v2.6.0.js').then((response) => response.text()),
  ...
  assert.match(fetchedAssets.manifest.name, /2\.6\.0/);
  assert.match(fetchedAssets.config, /version:\s*'2\.6\.0'/);
  assert.match(fetchedAssets.serviceWorker, /const CACHE_PREFIX = 'ilan-roadbook-v260-'/);
  ...
  const multidayJourneys = await verifyJourneys(page);
  ...
  assert.match(readyPass, /ספר הטיולים של אילן · גרסה 2\.6\.0/);
  ...
  assert.match(warningExport.filename, /2\.6\.0.*\.html$/);
  ...
  assert.match(routeExport.filename, /2\.6\.0.*\.html$/);
  ...
  assert.match(calendarDownload.filename, /2\.6\.0\.ics$/);
  ...
  assert.match(combinedExport.filename, /2\.6\.0.*\.html$/);
  ...
  if (WRITE) {
    ...
    await page.screenshot({ path: path.join(ROOT, 'reports', 'browser-mobile-2.6.0.png'), fullPage: false });
  }
  await page.locator('[data-close-dialog]').click();
  ...
  const cacheInfo = await page.evaluate(async () => {
    ...
    const activeKey = keys.find((key) => key === 'ilan-roadbook-v260-build-1');
  ...
  assert.equal(cacheInfo.activeKey, 'ilan-roadbook-v260-build-1');
  for (const expected of [
    'manifest-2.6.0.webmanifest', 'offline-2.6.0.html', 'assets/app-v2.6.0.css',
    'assets/app-v2.6.0.js', 'data/config-v2.6.0.js', 'data/route-expansion-v2.3.0.js',
  ...
  assert.match(await page.title(), /2\.6\.0/);
  assert.equal(await page.locator('#statRoutes').textContent(), '90');
  ...
  const result = {
    ...
    multiday_journeys: multidayJourneys,
    enlarged_map: true,
  ...
  if (WRITE) {
    fs.mkdirSync(path.join(ROOT, 'reports'), { recursive: true });
    fs.writeFileSync(path.join(ROOT, 'reports', 'browser-qa-2.6.0.json'), `${JSON.stringify(result, null, 2)}\n`, 'utf8');
  }
