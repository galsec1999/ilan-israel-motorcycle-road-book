...
(() => {
  ...
  function normalizeRoute(route, index) {
    ...
    const searchable = [
      released.title, released.region, released.area, released.duration, released.km, released.style,
  ...
  function exportCombinedPlan() {
    ...
    const rows = selected.map((route, index) => { const meetings = getMeetings(route); return `<section><h2>${index + 1}. ${escapeHtml(route.title)}</h2><p>${escapeHtml(route.start)} ← ${escapeHtml(route.end)} · ${escapeHtml(route.km)}</p><p>${escapeHtml(meetings.primaryPlace)} · יציאה ${escapeHtml(meetings.primaryDepart)}</p><a class="button" href="${escapeHtml(approachMapsUrl(route, meetings))}">מפת גישה למפגש</a><a class="button" href="${escapeHtml(mapsUrl(route))}">מפת מסלול הטיול</a></section>`; }).join('');
  ...
  function openRoute(routeId) {
    ...
    const qualityChecks = Object.entries(checkLabels).map(([key, name]) => {
      ...
      return `<div class="quality-check">${passed ? '✓' : '—'} ${escapeHtml(name)}</div>`;
    }).join('');
    ...
    <section class="detail-section quality-panel"><div class="quality-head"><div><h3>ביקורת שחרור טכנית</h3><strong>${escapeHtml(route.release_audit_status)}</strong></div><div class="quality-score">${releasePassed ? 'עבר' : 'בתהליך'}</div></div><div class="quality-checks">${qualityChecks}</div><p><strong>מועד הביקורת:</strong> ${escapeHtml(route.release_audited_on)}. <strong>מעמד התוכן:</strong> ${escapeHtml(route.verification_level)}. ${escapeHtml(route.verification_note || '')}</p></section>
