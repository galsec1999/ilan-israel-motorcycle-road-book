...
function evaluateAll() {
  ...
  for (const relative of [
    'data/config-v2.2.3.js',
    'data/legacy-content-v2.js',
    'data/new-routes-v2.js',
    'data/release-audit-v2.2.3.js',
...
const manifest = JSON.parse(read('manifest-2.2.3.webmanifest'));
const index = read('index.html');
const app = read('assets/app-v2.2.3.js');
const css = read('assets/app-v2.2.3.css');
...
test('גרסה 2.2.3 משמרת את כל חומר המקור ואת צילומי 2.2.0–2.2.2', () => {
...
test('דוח הביקורת מפרסם 90 מסלולים בשני טאבים ומשאיר רק מחקר לא גמור מוסתר', async () => {
  ...
  assert.equal(report.document_version, '2.2.3');
...
test('דוחות 2.2.3 הכתובים כוללים בדיקת רשת מלאה ושומרים את דוחות 2.2.1–2.2.2', () => {
  const jsonPath = path.join(root, 'reports', 'route-release-audit-2.2.3.json');
  const markdownPath = path.join(root, 'reports', 'ROUTE_RELEASE_AUDIT_2_2_3.md');
  assert.ok(fs.existsSync(jsonPath));
  assert.ok(fs.existsSync(markdownPath));
  const written = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
  assert.equal(written.document_version, '2.2.3');
  ...
  assert.match(markdown, /גרסה 2\.2\.3/);
...
test('HTML מציג גרסה אחת, טאב אזהרות, בטיחות וכל התצוגות', () => {
  ...
  assert.match(index, /data\/release-audit-v2\.2\.3\.js/);
  assert.match(index, /data\/config-v2\.2\.3\.js/);
  assert.match(index, /assets\/app-v2\.2\.3\.js/);
  assert.match(index, /assets\/app-v2\.2\.3\.css/);
  assert.match(index, /manifest-2\.2\.3\.webmanifest/);
...
test('כל תחנה מאפשרת להעתיק פרומפט עשיר ל־AI בלי להציג אותו', () => {
  ...
  assert.match(copyAction, /copyWithFeedback\(button, buildStopAiPrompt\(route, stop\)/);
...
test('manifest ו-Service Worker יחסיים, מתקינים ומסונכרנים לגרסה', () => {
  assert.equal(config.version, '2.2.3');
  assert.equal(manifest.version, '2.2.3');
  assert.match(manifest.name, /גרסה 2\.2\.3/);
  assert.match(manifest.short_name, /2\.2\.3/);
  ...
  assert.match(serviceWorker, /v2\.2\.3/);
  assert.match(serviceWorker, /data\/release-audit-v2\.2\.3\.js/);
  assert.match(serviceWorker, /assets\/app-v2\.2\.3\.js/);
  assert.match(serviceWorker, /offline-2\.2\.3\.html/);
...
test('robots, מעטפת offline וקובצי הפרסום שלמים', () => {
  ...
  assert.match(read('offline-2.2.3.html'), /גרסה 2\.2\.3/);
  for (const relative of [
    'index.html', 'offline-2.2.3.html', 'manifest-2.2.3.webmanifest', 'sw.js',
    'assets/app-v2.2.3.css', 'assets/app-v2.2.3.js', 'data/config-v2.2.3.js',
    'data/legacy-content-v2.js', 'data/new-routes-v2.js', 'data/release-audit-v2.2.3.js',
  ...
  for (const [compatibility, active] of [
    ['assets/app-v2.js', 'assets/app-v2.2.3.js'],
    ['assets/app-v2.css', 'assets/app-v2.2.3.css'],
    ['data/config-v2.js', 'data/config-v2.2.3.js'],
    ['data/release-audit-v2.js', 'data/release-audit-v2.2.3.js'],
    ['manifest.webmanifest', 'manifest-2.2.3.webmanifest'],
    ['offline.html', 'offline-2.2.3.html'],
