/**
 * Service Worker — גרסה 2.2.3
 * מוחק רק caches של הספר ואינו שומר מפות, AI או שמע דינמי.
 */

const CACHE_PREFIX = 'ilan-road-book-';
const CACHE_NAME = `${CACHE_PREFIX}v2.2.3-build-2`;
const APP_FILES = [
  './',
  './index.html',
  './manifest-2.2.3.webmanifest',
  './offline-2.2.3.html',
  './assets/app-v2.2.3.css',
  './assets/app-v2.2.3.js',
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith((async () => {
      try {
        ...
        return (await caches.match('./index.html')) || (await caches.match('./offline-2.2.3.html'));
      }
    })());
    return;
  }
