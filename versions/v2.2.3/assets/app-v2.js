...
(() => {
  ...
  async function copyWithFeedback(button, text, idleLabel, successLabel = 'הועתק ✓') {
    ...
    if (status) {
      status.textContent = '';
      requestAnimationFrame(() => { status.textContent = feedback; });
    }
  ...
  function buildStopAiPrompt(route, stop) {
    ...
    const routeIssue = route.release_has_issue
      ? `\n\nאזהרת ביקורת למסלול — חובה להתייחס אליה ולא להחליק אותה:\n${route.release_issue_reason}`
      : '';
    const sources = route.sources.length
      ? route.sources.map((source, index) => `${index + 1}. ${source}`).join('\n')
      : 'לא צורפו מקורות כלליים למסלול.';
...
הפרד בבירור בין עובדות מאומתות, מסקנות והמלצות. בטיחות, חוק והנחיות רשמיות עדכניות קודמים לתכנית הטיול.`;
  ...
  async function copyStopAiPrompt(button) {
    ...
    await copyWithFeedback(button, buildStopAiPrompt(route, stop), 'העתקה ל־AI', 'הועתק ל־AI ✓');
  }
